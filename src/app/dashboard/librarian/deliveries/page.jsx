...
//     // 🛠️ ফিক্স ১: আগের পেজের সাথে মিল রেখে BASE_URL অথবা API_URL ডাইনামিক করা হলো
//     const API_URL = process.env.NEXT_PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";
...
//             // 🛠️ ফিক্স ২: কুয়েরি প্যারামিটারে 'email' এবং 'librarianEmail' দুটোই পাঠানো হলো যেন ব্যাকএন্ড যেকোনো একটি ধরলেই ডাটা পায়
//             const res = await fetch(`${API_URL}/api/librarian/deliveries?email=${userEmail}&librarianEmail=${userEmail}`, {
//                 headers: {
//                     "Authorization": `Bearer ${token}`
//                 }
//             });
...
//             // 🛠️ ফিক্স ৩: ব্যাকএন্ড রেসপন্স ফরম্যাট ফিল্টারিং (result নিজেই অ্যারে হতে পারে অথবা অবজেক্ট)
//             let rawList = [];
//             if (Array.isArray(result)) {
//                 rawList = result;
//             } else if (result && typeof result === 'object') {
//                 rawList = result.data || result.deliveries || (result.success && result.orders) || [];
//             }

//             if (rawList.length > 0 || (result && result.success)) {
//                 const cleanList = rawList.map(order => {
//                     const orderId = order.id || order._id?.toString() || order._id;
//                     return {
//                         ...order,
...
//                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
//                         {deliveries.map((order) => (
//                             <div key={order.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex flex-col justify-between gap-4">
//                                 <div className="space-y-2">
//                                     <div className="flex justify-between items-start gap-2">
//                                         <div>
//                                             <div className="font-semibold text-gray-800 text-sm">{order.userName || "Unknown Client"}</div>
//                                             <div className="text-xs text-gray-400">{order.userEmail || "No Email"}</div>
//                                         </div>
//                                         <span className={getStatusBadgeClass(order.status)}>
//                                             {order.status ? order.status.charAt(0).toUpperCase() + order.status.slice(1) : "Pending"}
//                                         </span>
...
//                                     <div>
//                                         <span className="text-[10px] uppercase font-bold text-gray-400 block mb-0.5">Book</span>
//                                         <div className="font-medium text-gray-800 text-sm line-clamp-2">{order.bookTitle || order.title}</div>
//                                     </div>
//                                     <div>
//                                         <span className="text-[10px] uppercase font-bold text-gray-400 block mb-0.5">Date</span>
//                                         <div className="text-gray-600 text-xs">
//                                             {order.date || order.createdAt ? new Date(order.date || order.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : "N/A"}
//                                         </div>
//                                     </div>
//                                 </div>
//                                 <div className="pt-2 border-t border-gray-50 flex justify-end">
//                                     {(order.status === "pending" || !order.status) && (
//                                         <button disabled={updatingId === order.id} onClick={() => handleStatusChange(order.id, "Dispatched")} className="w-full px-4 py-2.5 text-xs font-semibold bg-[#4F46E5] hover:bg-[#4338CA] text-white rounded-lg shadow-md transition-all active:scale-95 disabled:opacity-50">
//                                             {updatingId === order.id ? "Processing..." : "Mark Dispatched"}
//                                         </button>
//                                     )}
//                                     {order.status === "dispatched" && (
//                                         <button disabled={updatingId === order.id} onClick={() => handleStatusChange(order.id, "Delivered")} className="w-full px-4 py-2.5 text-xs font-semibold bg-[#0EA5E9] hover:bg-[#0284C7] text-white rounded-lg shadow-md transition-all active:scale-95 disabled:opacity-50">
//                                             {updatingId === order.id ? "Processing..." : "Mark Delivered"}
//                                         </button>
//                                     )}
//                                     {order.status === "delivered" && (
...
//                             <tbody>
//                                 {deliveries.map((order) => (
//                                     <tr key={order.id} className="bg-white rounded-xl shadow-sm border border-gray-50 hover:bg-gray-50/50 transition-all duration-150">
//                                         <td className="py-5 pl-6 rounded-l-xl">
//                                             <div className="font-semibold text-gray-800 text-[14px]">{order.userName || "Unknown Client"}</div>
//                                             <div className="text-xs text-gray-400 mt-0.5">{order.userEmail || "No Email"}</div>
//                                         </td>
//                                         <td className="py-5 pr-4">
//                                             <div className="font-medium text-gray-800 text-[14px] line-clamp-1">{order.bookTitle || order.title}</div>
//                                         </td>
//                                         <td className="py-5 text-gray-600 text-[14px]">
//                                             {order.date || order.createdAt ? new Date(order.date || order.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : "N/A"}
//                                         </td>
...
//                                         <td className="py-5 pr-6 rounded-r-xl">
//                                             {(order.status === "pending" || !order.status) && (
//                                                 <button disabled={updatingId === order.id} onClick={() => handleStatusChange(order.id, "Dispatched")} className="px-4 py-2 text-xs font-semibold bg-[#4F46E5] hover:bg-[#4338CA] text-white rounded-lg shadow-md transition-all active:scale-95 disabled:opacity-50">
//                                                     {updatingId === order.id ? "Processing..." : "Mark Dispatched"}
//                                                 </button>
//                                             )}
//                                             {order.status === "dispatched" && (
//                                                 <button disabled={updatingId === order.id} onClick={() => handleStatusChange(order.id, "Delivered")} className="px-4 py-2 text-xs font-semibold bg-[#0EA5E9] hover:bg-[#0284C7] text-white rounded-lg shadow-md transition-all active:scale-95 disabled:opacity-50">
//                                                     {updatingId === order.id ? "Processing..." : "Mark Delivered"}
//                                                 </button>
//                                             )}
//                                             {order.status === "delivered" && (
